...
def newton_method_system(F, x0, eps=1e-5, max_iters=MAX_ITERS, log=True):
    ...
    for i in range(1, max_iters + 1):
        ...
        # Решаем систему J * delta = -F(x) с помощью метода Гаусса
        try:
            delta = gaussian_elimination(J, [-f for f in Fx])  # Решение системы
        ...
        if log:
            print(f"{i}: x = {x}, F(x) = {Fx}, ||delta|| = {math.sqrt(sum(d**2 for d in delta)):.6g}")

        # Если изменение слишком маленькое, выходим
